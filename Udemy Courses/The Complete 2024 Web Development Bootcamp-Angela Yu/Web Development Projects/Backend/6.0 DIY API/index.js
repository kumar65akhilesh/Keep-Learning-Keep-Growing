...
app.get("/jokes/:id", (req, res) => {
  ...
  jokes.forEach( (joke) => 
    {
      if(!found && joke["id"] == req.params["id"]){
        res.json(joke);
        found = true;   
      }
    }
...
app.get("/filter", (req, res)=>{
  ...
  const jokesArr =
      jokes.filter( (joke) => joke.jokeType == req.query["type"] );
...
app.post("/jokes" , (req, res) => {
  ...
  //console.log(jokes[jokes.length-1].id);
  //jokes[jokes.length-1];
  const id1 = jokes[jokes.length-1].id+1;
  jokes.push({"id":id1, "jokeType":req.body["jokeType"],"jokeText":req.body["jokeText"]});
...
app.put("/jokes/:id" , (req, res) => {
  ...
  const j = jokes.find((joke)=>joke.id === id1 );
  console.log(j);
  j.jokeText = req.query["text"];
  j.jokeType = req.query["type"];
  res.json(j);
});
...
app.delete("/jokes/:id" , (req, res) => {
  const id1 = parseInt(req.params["id"]);
  const j = jokes.findIndex((joke)=>joke.id === id1 );
  console.log(j);
  if(j > -1) {
    jokes.splice(j,1);
    res.sendStatus(200);
